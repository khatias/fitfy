...
function HeaderBottom() {
  ...
  return (
    <div className="hidden lg:flex m-auto max-w-[1440px] lg:py-2 px-6 border-b border-gray-200 bg-white dark:bg-black">
      <nav aria-label="Product Categories" className="w-full">
        <ul className="flex space-x-8 w-full">
          {categories.map((category) => (
            <li className="py-3 group relative" key={category.product_category_id}> {/* Added group class here */}
              <Link
